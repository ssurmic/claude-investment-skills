...
> 🔧 源文件：[`diagrams/architecture-zh.mmd`](./diagrams/architecture-zh.mmd) —— 改 `.mmd` 文件然后 push，`.svg`/`.png` 会通过 [`render-diagrams.yml`](./.github/workflows/render-diagrams.yml) 自动重新生成。本地工作流见 [README → Diagrams pipeline](./README.md#-diagrams-pipeline-mermaid--svg)。
